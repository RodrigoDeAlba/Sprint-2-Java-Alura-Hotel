...
public class Busqueda extends JFrame {
			...
			public void mouseClicked (MouseEvent e) {
			    ...
			    if(filaReservas >= 0) {
			    	...
			    	if(confirmar == JOptionPane.YES_OPTION) {
			    		...
			    		JOptionPane.showMessageDialog(contentPane, "Registro borrado");
			    		LimpiarRegistro();
			    		LlenadoTablaR();
			    		LlenadoTablaH();
			    	}
			    	...
			    	if(confirmarH == JOptionPane.YES_OPTION) {
			    		String valor = tbReservas.getValueAt(filaHuespedes, 0).toString();
	...
	private void LlenadoTablaHId() {
		List<Huesped> huesped = buscarHuespedId();
		try {
			for (Huesped huespedes : huesped) {
				modeloH.addRow(new Object[] { huespedes.getId(), huespedes.getNombre(), huespedes.getApellido(), huespedes.getFechaNacimiento(), huespedes.getNacionalidad(),huespedes.getTelefono(),huespedes.getIdReserva()});
			}
	...
	private void ModificarHuesped() {
		...
		.ifPresentOrElse(filaH -> {
			...
			this.huespedesController.modificar(nombre, apellido, fechaNacimiento, nacionalidad, telefono, idReserva, id);
			JOptionPane.showMessageDialog(this, String.format("Registro modificado con éxito"));
		}, () -> JOptionPane.showMessageDialog(this, "Por  favor elije un registro"));
